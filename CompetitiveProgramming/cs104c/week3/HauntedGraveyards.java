 ...
 */
public class HauntedGraveyards
{
    ...
    boolean relax()
	{
        ...
        for (int i = 0; i < R; ++i)
		{
            for (int j = 0; j < C; ++j)
			{
                if (holes[i][j] || graves[i][j] || (i == R - 1 && j == C - 1) || distance[i][j] == INF)
                    continue;
                for (int d = 0; d < 4; d++) {
                    int newI = i + dx[d];
                    int newJ = j + dy[d];
        ...
        for (int i = 0; i < E; ++i)
		{
			if (distance[portals[1][i]][portals[0][i]] == INF) continue;
            if (distance[portals[1][i]][portals[0][i]] + portals[4][i] < distance[portals[3][i]][portals[2][i]])
			{
                relaxed = true;
